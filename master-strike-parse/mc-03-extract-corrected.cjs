...
console.log('🎯 Processing master-strike data with CORRECT duplicate handling...\n');
...
function processEntitiesCorrectly(entities, entityType, fileSetId) {
  ...
  for (let entity of entities) {
    ...
    if (entity.set && Array.isArray(entity.set) && entity.set.length > 1) {
      ...
      console.log(`    🎯 TRUE DUPLICATE: ${entity.name} → sets: ${entity.set.join(', ')}`);
  ...
  return { entities: processedEntities, duplicateCount: duplicateCount - (duplicateCount > 0 ? entities.filter(e => e.set && Array.isArray(e.set) && e.set.length > 1).length : 0) };
}
...
(async () => {
  ...
  // Process card files
  const cardsDir = path.join(__dirname, 'master-strike/packages/data/src/definitions/cards');
  const cardFiles = fs.readdirSync(cardsDir)
    .filter(file => file.endsWith('.ts') && file !== 'index.ts')
    .map(file => ({ name: path.basename(file, '.ts'), path: path.join(cardsDir, file) }));
  ...
  for (let { name: fileName, path: filePath } of cardFiles) {
    ...
    const entityTypes = ['heroes', 'masterminds', 'villains', 'henchmen', 'schemes', 'bystanders'];
    
    for (let entityType of entityTypes) {
      if (data[entityType]) {
        const result = processEntitiesCorrectly(data[entityType], entityType, fileSetId);
        ...
        if (result.entities.length > 0) {
          ...
          if (duplicatesCreated > 0) {
            console.log(`  ✅ ${entityType}: ${originalCount} → ${finalCount} (+${duplicatesCreated} duplicates)`);
