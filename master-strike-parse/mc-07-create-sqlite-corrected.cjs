...
(async () => {
  ...
  for (let entityType of entityTypes) {
    if (nedbInstances[entityType]) {
      ...
      entities.forEach(entity => {
        const key = `${entity.name}|${entity.id}|${JSON.stringify(entity.cards?.map(c => c.name).sort())}`;
        if (!entityGroups[key]) {
          entityGroups[key] = [];
        }
      ...
      const trueDuplicates = Object.entries(entityGroups)
      ...
      if (trueDuplicates.length > 0) {
        ...
        trueDuplicates.forEach(([key, group]) => {
          ...
          console.log(`    - ${entity.name}: ${group.length} records in sets [${sets.join(', ')}]`);
        });
  ...
  howardRecords.forEach((row, index) => {
    ...
    console.log(`    Record ${index + 1}: Set ${record.set}, Cards: ${record.cards?.length || 0}`);
  });
  ...
  allDuplicates.forEach(row => {
    console.log(`  ${row.list}: ${row.name} (${row.count} records in sets [${row.sets}])`);
  });
